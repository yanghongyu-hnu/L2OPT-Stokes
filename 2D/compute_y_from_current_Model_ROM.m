...
result.Yu_hat = sparse(N_xi*N_xi, para.n_ou * para.n_f);
result.Yp_hat = sparse(N_xi*N_xi, para.n_op * para.n_f);

for i = 1:N_xi
    start = (i-1) * N_xi;
    for j = 1:N_xi
        row = start + j;
        para.int_para.xi = [Xi(i,1); Xi(j,2)];
        ...
        result.Yu_hat(row,:) = yu_hat(:)';
        result.Yp_hat(row,:) = yp_hat(:)';
    end
end
end